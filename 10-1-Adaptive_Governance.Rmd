...
.textwhite[.size75[**Adaptive Governance**]]
...
- No country "owns" the Atlantic Ocean thus making it a "commons" or owned in common by the whole world.
...
Per Ostrom, every CPR can be, and probably should be, analyzed in these two component pieces for at least two good reasons:

First, our aim should be a more complete understanding of each separately while also never forgetting that they are dependent on each other.

- The health of the system can certainly be evaluated separately from the key resource unit AND vice-versa

<br>

Second, it focuses our analyses onto more directly useful questions.
...
*Create new groups (3-4?)*

Ok, groups I want you to evaluate these principles.

As a policy designers, which is the best place to start when trying to implement an adaptive governance plan to address a CPR problem?
...
.size45[**Thursday: Adaptive Governance Cases**]
...
.size45[
.textblue[**Present as an argument**]: This case shows that addressing environmental problem X can be done successfully using adaptive governance.
]
